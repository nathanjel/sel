...
if(process.env.AUDIT_UNIQUE_KEY) bindings.R=bindings.R.withUniqueKey('id');
const replay=process.env.AUDIT_REPLAY_DATA?.trimEnd().split('\n')??(process.env.AUDIT_REPLAY?readFileSync('tools/adversarial/replay-js.sel','utf8').trimEnd().split('\n'):null);
let n=0;
for(const [i,source] of (process.env.AUDIT_QUERY??readFileSync('tools/adversarial/queries.sel','utf8')).trimEnd().split('\n').entries()) {
 for(const d of ['sqlite','postgresql','mariadb']) for(const strict of [false,true]) {
  ...
  try {const p1=Sql.planHybrid(p,d,bindings,{strict});out.plan=p1.pureSql?'pure_sql':p1.pureMemory?'pure_memory':'hybrid';out.prefix=p1.sqlStatement?.asStatement()??null;if(p1.sqlStatement)Object.assign(out,{prefix_params_sql:p1.sqlStatement.asStatement('params'),prefix_params:p1.sqlStatement.bindings().map(v=>v.asText())});} catch(e){out.plan_error=e.code??String(e);}
