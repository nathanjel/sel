...
// that is the same (the pre-filter; the conformance cases hold it there).
for (const [src, why] of [
  ['ORDERS .> LINK(CUSTOMERS, _1["customer_id"] == _2["id"])'
    + ' .> FILTER(_["orders"]["status"] $== "ACTIVE" AND _["customers"]["country"] $== "DE")',
    'a left and a right conjunct'],
  ['ORDERS .> LINK(CUSTOMERS, _1["customer_id"] == _2["id"]) .> FILTER(_["customers"]["country"] $== "DE")',
    'a right conjunct'],
  ['ORDERS .> LINK_LEFT(CUSTOMERS, _1["customer_id"] == _2["id"]) .> FILTER(_["orders"]["status"] $== "ACTIVE")',
    'a left conjunct over LINK_LEFT'],
]) {
  const steps = optimizedSteps(src, true);
  same(names(steps), [src.includes('LINK_LEFT') ? 'LINK_LEFT' : 'LINK', 'FILTER'], `no FILTER crosses a LINK: ${why}`);
  check(steps[0].args[1].t === 'var', `the LINK's right side is untouched: ${why}`);
}
same(names(optimizedSteps(
  'ORDERS .> FILTER(_["status"] $== "ACTIVE")'
  + ' .> LINK(CUSTOMERS, _1["customer_id"] == _2["id"])'
  + ' .> FILTER(_["orders"]["status"] $== "ACTIVE")', true)), ['FILTER', 'LINK', 'FILTER'],
...
{
  const steps = optimizedSteps('ORDERS .> LINK(CUSTOMERS, _1["customer_id"] == _2["id"])'
    + ' .> FILTER(_["orders"]["status"] $== "ACTIVE") .> MAP(1)', true);
  check(steps[1].args[1].keysUnobserved === true, 'a FILTER followed by a MAP has unobserved keys');
  const last = optimizedSteps('ORDERS .> LINK(CUSTOMERS, _1["customer_id"] == _2["id"])'
    + ' .> FILTER(_["orders"]["status"] $== "ACTIVE")', true);
  check(last[1].args[1].keysUnobserved === false, 'a FILTER that ends the pipeline has observed keys');
}
