...
This document is the design for the SQL layer; [SEL and SQL](../sql.md) is its
user-facing summary. It is written in the same register as `spec/SPEC.md` —
where it and a future implementation disagree, resolve it here first — but it is
*not* part of
...
The retired SQL-TESTING analysis (in the git history) §2 has the table.

**A corpus of symmetric inputs cannot see an asymmetric bug.** This is §4 of
The retired SQL-TESTING analysis (in the git history) and it keeps recurring in new clothing. The sharpest
...
The delivery plan (milestones M1–M7, the alpha criteria, what was ruled out of
scope) and the analysis behind the test strategy — seven classes of check that
could not fail, and what was built to fix each — were retired design notes
(`docs/history/SQL-DELIVERY.md` and `SQL-TESTING.md`). They are in the git
history; this document describes what exists. Code comments that cite "§14 M7"
refer to the last milestone there: a differential SQL fuzzer, which is
`tools/fuzz-sql.sh`.
