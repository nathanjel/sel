...
Both differential fuzzers draw programs from tools/gen-programs.mjs, whose only relational vocabulary is `ALL/ANY/MAP/FILTER/SUM` spelled as plain calls (line 154); it never emits `.>`, BUCKET, SORT_BY/TOP_BY, TAKE/DROP, LINK, or a 3-argument SORT_BY. 2000 generated programs (seed 11) contain zero of those tokens. The SQL fuzzer therefore never exercises translate_statement/plan_hybrid (the runners lens already noted 'no pipelines generated'), and the language fuzzer cannot see the SORT_BY-form flip (finding A), the bare-bucket key rendering (G), the C++ `_K` ordinal (Q), the LAZY_RECORD strictness split (R) or the LINK duplicate-key rows (Y) — every cross-host divergence this review found lives in a shape the generator cannot produce. sql/mutations.json (161 entries, unchanged by the commit) contains no mutation of the planner, the bucket state machine, bucketRowsAreKeys, unwind/build_pipeline or the fold arms, so tools/mutate-sql.sh cannot say whether the 33 new plan fixtures would notice a broken planner. The fixtures do have teeth — mutating JS bucketRowsAreKeys to `return false` fails 3 of 33 plan cases — but only for what a hand-written case already names; the commit's EXTENDING guardrail 'the fuzzer compares positions, and so must any fold you add' and CHANGELOG 'This was the one disagreement tools/check.sh's default fuzz seed had been finding' describe scalar folds only. Pre-existing; the commit added no generator vocabulary or mutation entries while moving most of its risk into exactly these lanes.

## Reported repro

```
cd /home/nathan/workspaces/nth-share/sel && node tools/gen-programs.mjs 2000 11 | grep -c '\.>\|BUCKET\|SORT_BY\|TAKE\|LINK'   # 0 ; node tools/gen-programs.mjs 2000 11 | grep -c 'FILTER\|MAP'  # 76 (as calls). grep -n 'hybrid\|planner\|bucket\|BUCKET' sql/mutations.json  # no output. Hand mutation: cp -r js sql <scratch>/repo; in <scratch>/repo/js/src/sql/hybrid.mjs change line 124 to `function bucketRowsAreKeys(steps) { return false;` ; cd <scratch>/repo && node js/bin/sqlt.mjs plan.  -> '30 passed …, 3 failed' (plan.bucket.split-before-the-bucket, plan.bucket.sealed-prefix-is-not-a-split-point, one 'expected a pure_memory plan, got hybrid').
```
