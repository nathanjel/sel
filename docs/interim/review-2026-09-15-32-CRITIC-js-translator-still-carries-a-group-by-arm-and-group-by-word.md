...
CHANGELOG.md:28 says 'the registries, dependencies() and every translator now carry BUCKET alone'. js/src/sql/translator.mjs:1883 keeps `case 'GROUP_BY':` falling through to the BUCKET arm, and its two refusals read 'the binder of GROUP_BY must be a bare name' (1898) and 'GROUP_BY takes 2 to 4 arguments' (1904); the PHP, Python, C++ and Lisp translators contain no GROUP_BY text. The arm is unreachable (GROUP_BY is E_UNKNOWN_FUNC at compile time in all five hosts, so no BUCKET-shaped node named GROUP_BY can reach the translator) and error messages are not asserted, so nothing is observable today; it is dead code contradicting the written claim and the EXTENDING 'Vocabulary, again' rule that GROUP_BY 'does not appear in SEL'. The dead case predates the commit only in the sense that the commit removed the alias everywhere else and left this one.

## Reported repro

```
cd /home/nathan/workspaces/nth-share/sel && git grep -n GROUP_BY -- js/src php/src python/sel cpp/*.cpp lisp/src ':!*_map*' ':!*map-data*' ':!*MapData*'   # only js/src/sql/translator.mjs:1883,1898,1904 ; for each REPL: node js/bin/sel.mjs -e 'LIST(RECORD("c",1)) .> GROUP_BY(_["c"])' -> E_UNKNOWN_FUNC at line 1 column 24 (php, cpp, lisp, python identical).
```
