...
(defun analyze-sort-step (tr step plan)
  (let* ((name (sel::node-s step))
         (orig-args (sel::node-items step))
         (is-top (member name '("TOP" "TOP_DESC" "TOP_BY") :test #'equal))
         (args (if is-top (butlast orig-args) orig-args))
         (sort-name (cond ((equal name "TOP") "SORT")
                          ((equal name "TOP_DESC") "SORT_DESC")
                          ((equal name "TOP_BY") "SORT_BY")
                          (t name)))
         (count (length args))
         (pos (snode-pos step)))
    (when is-top
      (let ((lim (eval-int-param tr (car (last orig-args)) name)))
...
(defun plan-output-fields (plan sub-alias)
  (cond
    ((relational-plan-projections plan)
     (let ((fields '()))
       (dolist (p (relational-plan-projections plan))
         (let* ((col-name (or (first p)
                              (when (and (third p)
                                         (not (clist-p (third p)))
