    ...
    (case (node-kind node)
      ...
      (:call
       (let ((spec (node-spec node))
             ...
             (name (node-s node)))
         (cond
           ((member name '("SORT" "SORT_DESC" "SORT_BY") :test #'string=)
            (let ((n (length items)))
              (cond
                ((= n 1)
                ...
                ((and (= n 4) (eq (node-kind (second items)) :var))
                 ...
                 (collect-deps (fourth items) bound reads assigned (1+ depth)))
                ((and (= n 3) (eq (node-kind (second items)) :var))
                 (collect-deps (first items) bound reads assigned (1+ depth))
                 (let ((inner (copy-name-table bound)))
                   (setf (gethash (node-s (second items)) inner) t)
                   (setf (gethash "_K" inner) t)
                ...
                ((= n 3)
                 ...
                 (collect-deps (third items) bound reads assigned (1+ depth)))
                ((= n 2)
                 (collect-deps (first items) bound reads assigned (1+ depth))
                 (let ((inner (copy-name-table bound)))
                   (setf (gethash "_" inner) t)
              ...
              (cond
                ((and (= n 4) (eq (node-kind (second items)) :var))
                 ...
                 (let ((inner (copy-name-table bound)))
                   ...
                   (setf (gethash "_K" inner) t)
                   (collect-deps (third items) inner reads assigned (1+ depth))
                   (collect-deps (fourth items) inner reads assigned (1+ depth))))
                ((= n 3)
                 (collect-deps (first items) bound reads assigned (1+ depth))
                 (let ((inner (copy-name-table bound)))
                   (setf (gethash "_" inner) t)
