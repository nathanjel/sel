...
Scenario 6 leaves memory: an `IF` or `COND` of text literals keeps SEL's identity in SQL (2026-09-24; WL-001 SEL-0057).
  ...
  - **The rule.** An `IF`/`COND` whose every result is a text literal (or in turn such a conditional; a two-argument `IF`'s `""` counts) passes the identity proof that `DEDUPE`, `BUCKET` and the planner's barrier share. SQL's `CASE` returns the literal it chose byte for byte, including `'a'`/`'A'` and `'a'`/`'a '` on PostgreSQL 17 and MariaDB 11.8. Numeric results stay refused, since MariaDB answers `1.0` for `IF(c, 1, 1.0)`'s `1`, as do columns and computations. `CANON` is how a rule makes a numeric result provable.
  - **Scenario 6** now splits after its `MAP`: SQL returns the 100 unsold products and memory runs `DEDUPE`, sort and `TAKE`. The database lane measured 24 ms on PostgreSQL and 8 ms on MariaDB, against about 2,180 ms before, with parity passing on both. The reference was regenerated by the Lisp generator (only S6's SQL fields changed), and `PURE_MEMORY_REASONS` is empty.

Validation: 973 SQL cases per host (16 new, two flipped); the oracle 0 differ on all four servers, with three new statements; 222 mutations, all caught (6 new); `tools/check.sh` ALL GREEN on seven roster entries with the gate's Docker servers (753 s).

`CANON(x)`: the canonical form of a number, so a rule can compare numbers by value where SEL compares by identity, and SQL can prove it (2026-09-24; WL-001 SEL-0058, SEL-0059, SEL-0060).

  - **The language.** Scale is part of a number and so of its identity: `DEDUPE`, `BUCKET`, `EQL`, `IN` and index keys see `1`, `1.0` and `1.00` as three values. `CANON` gives every number the one spelling all equal numbers share, the §4.1 form without the fraction's trailing zeros: `CANON(1.50)` is `1.5`, `CANON(100)` is `100`, `CANON(-0.00)` is `0`. `LIST(0.5 + 0.5, 4 / 4) .> MAP(CANON(_)) .> DEDUPE()` has one element. It reads its argument as `ABS` does, and returns an ordinary number. It is implemented in every host's decimal core as `trim_scale`.
  - **The SQL spellings were verified on four servers first:**
    - PostgreSQL uses `trim_scale`, and the result is a number.
    - The MySQL family trims the value's text with a regex chain written without backreferences, which MariaDB and MySQL spell differently.
    - SQLite trims the number's text, with its usual `decimal-float` caveat.
    - ANSI uses `CAST`/`POSITION`/`TRIM`.

    Each was checked over typed columns, arithmetic, the guard and literals on PostgreSQL 17, MariaDB 11.8, MySQL 8.4 and SQLite 3.51.
  - **What it unlocks.** `DISTINCT` over a numeric column, and grouping by a computed numeric key, were refused, because SQL merges `1` and `1.0` and SEL does not. Over `CANON` both are exact, and the planner's identity barrier lets `CANON` through without asking about the fields it reads. Scenario 6's shape with `CANON` is one statement on PostgreSQL. A `CANON` the dialect can only carry as text is refused as a sort key, and the planner sorts it in memory.
  - **`Fragment.canonical`** is public, and the oracle compares a canonical answer as text. The flag was dropped by every host's top-level `translate` at once; parity missed it, so `tools/check-sqlapi.sh` now pins values as well as comparing hosts, and the mutation lane runs it.
  - **scale-limit on the MySQL family's numeric casts (SEL-0059).** `numericCast`/`numericGuard` are `DECIMAL(65,10)`, and a text number past ten fractional digits silently lost them (`1.00000000001` became `1.0000000000` on MariaDB 11.8). The dialect now declares the cap as `numericCastScale`, and the generator ties it to the SQL. A value read through the cap carries `scale-limit` and is refused under strict: a guarded operand, a column in a coerce comparison, or a constant past ten digits.
  - **text-order on text sort keys (SEL-0060).** SEL sorts number-shaped text as numbers and SQL sorts a text key by its bytes, so `10, 9, 100` sorted `9, 10, 100` in SEL and `10, 100, 9` on every server, with nothing said. Every sort by a TEXT or UNKNOWN key now carries the new `text-order` caveat and is refused under strict, and the planner sorts in memory then.

Validation: 1027 conformance cases per host (33 new); 957 SQL cases per host (64 new); the oracle 0 differ on all four servers and the ANSI probe, with the two new caveats witnessed by statements that must differ; 216 SQL mutations (19 new), all caught; `tools/check.sh` ALL GREEN on seven roster entries with the gate's Docker servers (4621 s wall time, about an hour of it blocked on a slot lock an interrupted mutation run of mine had orphaned — see the `NOT_LEAF` note in `tools/mutate-sql.py` for the deadlock that left it).

The scale harness's database lanes run all six scenarios again, and a scenario the planner keeps in memory needs a reason on record (2026-09-23; WL-001 SEL-0055).
...
Python builds a left row's joined rows in one loop and gets scenario 6 back (2026-09-23; WL-001 SEL-0056).

  - **One call per left row, not per pair.** Each compiled joined-row plan now comes with a generated loop over a left row's matches. The loop checks each right element's shape and the plan's guards, and sends a pair that breaks them the general way. Before, each pair cost two Python calls (the projector, then the plan), and S6's 90,000 right rows each meet one left row, so a pass over the right rows could not help.
  - **Cheaper guards.** Fields that must not be records are tested together first, `"NONE" not in (kinds…)`, and one by one only when that fails.
  - **Cost, three interleaved rounds, seven runs each:** S6 −5.3% against the tree before, level with `0f9031d` (789–795 ms against 781–793). S1–S5 are within spread of the tree before. S1 stays 3.7% above `0f9031d`, and the residual is accepted: with every guard off it is level, and its joins meet one match per left row, which nothing short of per-row facts would amortise.

Validation: 994 conformance cases per host (two new ones on runs of matches that change category partway); pytest 636 (a new one on the loop's memo); the joined-row oracle 0 wrong; the join-filter oracle 0 disagreements; `tools/check.sh` ALL GREEN on seven roster entries (603 s) with the gate's Docker servers.

A FILTER after a LINK answers what the same program answers through a helper variable (2026-09-23; WL-001 SEL-0054).

  - **No rewrite moves a `FILTER` across a `LINK` any more.** Every host's physical optimiser moved a FILTER's one-sided conjuncts under the join (tentatively since SEL-0051), and on 49–66 of 3,000 generated join-then-filter programs every host answered differently from the same program with the join bound to a variable first: the joined rows were renumbered (`_K` and the result's keys), a join key that raises on a dropped row — this join's, or one above — was never computed, and under explicit binders the relation's name was taken for a member of the row. Spec §7.4 now says an early test changes none of these, ten `as-written.*` cases pin them, and the pushdown is gone from all five hosts with its tentative-FILTER marks and counter.
  - **The join does the work at run time instead.** Beside SEL-0052's left-row test it now tests, once per right row, a conjunct that reads only through its own right binder (`_["products"]["is_active"]`), keeping rejected rows for the numbering and for left rows kept on an error; and a `FILTER` skips the conjuncts its join applied without error. Two faults of the pre-filter the pushdown had hidden are fixed and pinned by `as-written.chain.*`: a middle FILTER's conjunct was judged against joins above it, and drops below could spare an upper join's right keys.
  - **The join-then-filter oracle is a gate lane**, `join then filter as written`, failing on any difference from the helper form or between hosts.
  - **Cost, `7e450e1` against this tree, interleaved:** every scenario on every host within run-to-run spread or faster; S5 1–5% faster on four hosts and faster on PHP on a recheck; Lisp and PHP needed their pre-filter loops tuned (binders first in Lisp's frame, and C++'s; rows tested before they are extended, and walked in place in PHP) to match the pushed FILTER on S3.

Validation: 992 conformance cases per host; pytest 635, JS optimizer 142, PHP optimizer 140, C++ unit 173 and SQL unit 85, Lisp 548, SQL cases 889; `make asan` clean; the join-filter oracle 0 disagreements on every corpus run; the joined-row oracle 0 wrong; `tools/check.sh` ALL GREEN on seven roster entries (610 s) with the gate's Docker servers, the new join-then-filter lane among its layers, 197 mutations caught.
